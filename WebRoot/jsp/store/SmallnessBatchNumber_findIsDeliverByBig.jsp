...
<div style="position: absolute;width: 100%;height: 100%;background:white;display: none;" id="wait" >
  	<div style="width:100%;height:100%;text-align: center;background: #dddddd;">
		...
		<div style="font-size: 14px;">当前位置&nbsp;>&nbsp;仓库物资管理&nbsp;>&nbsp;发货管理&nbsp;>&nbsp;发货详情</div>
			...
			%>
				...
				<tr>
					...
					<td>
						...
						%>
						<a href="javascript:modifyDate(<%=s.getId()%>);" >
						<%
					...
					<td>
						<%
							if(s.getIsDeliverGoods() == 1) {
						%>
							<a href="SmallnessBatchNumber!changeIsDeliverGoods.action?mergers=<%=s.getId() %>&state=2">发货</a>
						<%
							}else if(s.getIsDeliverGoods()==2){
						%>
							<a href="SmallnessBatchNumber!changeIsDeliverGoods.action?mergers=<%=s.getId() %>&state=1">取消</a>
						<%
							} else {
								out.print("&nbsp;");
							}
					...
					<td>
						<a href="javascript:print(<%=s.getId() %>)">打印</a>
					</td>
				</tr>
			<%	
				}
