...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<%
		 ...
		 %>
		<div style="font-size: 14px;">当前位置&nbsp;>&nbsp;生产单管理&nbsp;>&nbsp;大批号详情</div>
		<hr />
		<div>
			<table  cellpadding="1" cellspacing="1"   cellpadding="1" cellspacing="1"  cellpadding="1" cellspacing="1">
				...
				%>
					<tr>
						...
						<td>
							<a href="SmallnessBatchNumber!findFlowOperator.action?smallnessBatchNumberId=<%=s.getId() %>" target="mainFrame">
	...
	<form action="SmallnessBatchNumber!findFlowByBig.action" method="post" id="fenye">
		<table cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="font-size: 12px;">
			...
			<tr>
				...
				<td style="text-align: right;">
					...
					%>
							<a href="SmallnessBatchNumber!findFlowByBig.action?bigBatchNumberId=<%=bigBatchNumberId %>&page.currentPage=1&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">第一页</a>
					<%
						} else {
					%>
					...
					%>
							<a href="SmallnessBatchNumber!findFlowByBig.action?bigBatchNumberId=<%=bigBatchNumberId %>&page.currentPage=<%=p.getCurrentPage()-1 %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">上一页</a>
					<%
						}
					 %>
					...
					%>
							<a href="SmallnessBatchNumber!findFlowByBig.action?bigBatchNumberId=<%=bigBatchNumberId %>&page.currentPage=<%=p.getCurrentPage()+1 %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">下一页</a>
					<%						
						}					 
					  %>
					...
					%>
						<a href="SmallnessBatchNumber!findFlowByBig.action?bigBatchNumberId=<%=bigBatchNumberId %>&page.currentPage=<%=p.getLastPage() %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">最后一页</a>
					<%
						}
					 %>
