...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div style="font-size: 14px;">当前位置&nbsp;>&nbsp;<a href="WorkFlow!list.action?flowTypeId=5">质检流程</a></div>
		...
		<div>
			<table  cellpadding="1" cellspacing="1"  >
				...
				<tr>
					<td>
						<%
							if(w.getSmallnessBatchNumber().getIsDistribute() == 3 || w.getSmallnessBatchNumber().getIsDistribute() == 4 || w.getSmallnessBatchNumber().getIsRemakes() == 1) {
						%>
							<input name="smallnessBatchNumberIds" type="checkbox" disabled="disabled"  style="width: 25px;height:25px;"
						...
						<td>
							...
							<%
								if(w.getSmallnessBatchNumber().getIsDistribute() == 3) {
									out.print("补制中");
								} else if(w.getSmallnessBatchNumber().getIsDistribute() == 4) {
									out.print("审核中");
								} else if(w.getNextFlow().getId() == 7) {
									out.print("<span style='color:red;'>总质检返回</span>");	
								} else {
		...
		<table cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="font-size: 12px;">
			...
			<tr>
				<td style="text-align: left;">
					...
					<input type="button" class="button" value="待补区" onclick="findRemakes()"/>
