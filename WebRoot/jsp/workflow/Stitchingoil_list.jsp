...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
  	...
  	function setChkboxStatus(tableId)
		...
		//停制
		function stopfor(select){
			if(window.confirm("确定停制?")) {
				var selRow = document.getElementsByName(select);
				var str=false;
				for(var i=0;i<selRow.length;i++){
			    ...
			    if(str) {
			    	document.getElementById("myForm").action="BatchRefashion!stopfor.action";
					document.getElementById("myForm").submit();
			    } else {
			    	alert("请选择要停制的批号");
			    }
		...
		function isSelect(isSelect, removeSelect, remove2, remove3) {
			...
			var removes1 = document.getElementById(remove2);
			for(var j=0; j<removes1.options.length; j++) {
				if(removes1.options[j].value == name) {
					if(removes1.options[j].value != 0) {
						removes1.options.remove(j);
					}
				}
			}
			
			var removes3 = document.getElementById(remove3);
			for(var j=0; j<removes3.options.length; j++) {
				if(removes3.options[j].value == name) {
					if(removes3.options[j].value != 0) {
						removes3.options.remove(j);
					}
				}
			}
			...
			for(var i = 0; i < users.options.length; i++) {
				if(users.options[i].selected != true) {
					for(var j=0; j<removes1.options.length; j++) {
						if(users.options[i].value == removes1.options[j].value) {
							isTrue = false;
						}
					}
					if(isTrue) {
			...
			for(var i = 0; i < users.options.length; i++) {
				if(users.options[i].selected != true) {
					for(var j=0; j<removes3.options.length; j++) {
						if(users.options[i].value == removes3.options[j].value) {
							isTrue = false;
						}
					}
					if(isTrue) {
		...
		function goNextFlow(select){
			if(window.confirm("确定提交?")) {
			    ...
			    if(str) {
			    	document.getElementById("myForm").action="NumberPlate!goNextFlow.action";
					document.getElementById("myForm").submit();
			    } else {
			    	alert("请选择要提交的批号");
			    }
	...
	<body>
		<div style="font-size: 14px;">当前位置&nbsp;>&nbsp;<a href="WorkFlow!list.action?flowTypeId=3">滚油流程</a></div>
		...
		<div class="saixuan">
			员工名称：
			<select name="select" id="operator1" onchange="isSelect('operator1','operator2','operator3','operator4')">
			...
			<select name="select" id="operator2" onchange="isSelect('operator2','operator1','operator3','operator4')">
				<option value="0">--请选择--</option>
				<%
					for(Users u : users) {
				%>
			...
			<select name="select" id="operator3" onchange="isSelect('operator3','operator2','operator1','operator4')">
				<option value="0">--请选择--</option>
				<%
					for(Users u : users) {
				%>
			...
			<select name="select" id="operator4" onchange="isSelect('operator4','operator2','operator1','operator3')">
				<option value="0">--请选择--</option>
				<%
					for(Users u : users) {
				%>
		...
		<div>
			<table  cellpadding="1" cellspacing="1"  >
				...
				<tr>
					<td>
						<%
							if(w.getSmallnessBatchNumber().getIsDistribute() == 3 || w.getSmallnessBatchNumber().getIsDistribute() == 4 || w.getSmallnessBatchNumber().getIsDistribute() == 5) {
						%>
							<input style="width: 25px;height:25px;" name="smallnessBatchNumberIds" type="checkbox" disabled="disabled"
										value="<%=w.getSmallnessBatchNumber().getId() %>"
										id="chkid<%=w.getSmallnessBatchNumber().getId() %>" />
						<%
							} else {
						%>
							<input style="width: 25px;height:25px;" name="smallnessBatchNumberIds" type="checkbox"
										value="<%=w.getSmallnessBatchNumber().getId() %>"
										id="chkid<%=w.getSmallnessBatchNumber().getId() %>" />
						<%
							}
						...
						<td>
							<%
								if(w.getSmallnessBatchNumber().getIsDistribute() == 1) {
									...
									out.print("停制中&nbsp;<a href='BatchRefashion!celstopfor.action?flowTypeId=3&smallnessBatchNumber.id="+w.getSmallnessBatchNumber().getId()+"'>取消</a>");
								}
							 %>
						</td>
						<td>
		...
		<table cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="font-size: 12px;">
			...
			<tr>
				<td style="text-align: left;">
					<input type="button" class="button" value="提交下个流程" onclick="goNextFlow('smallnessBatchNumberIds')"/>
					<input type="button" class="button" value="整批重制" onclick="addRefashion('smallnessBatchNumberIds')"/>
					<input type="button" class="button" value="停制" onclick="stopfor('smallnessBatchNumberIds')"/>
				</td>
