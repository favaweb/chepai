...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<div style="font-size: 14px;">当前位置&nbsp;>&nbsp;<%if(plateType == 0)out.print("<a href='WorkFlow!list.action?flowTypeId=1'>反压流程</a>");else if(plateType == 1)out.print("<a href='WorkFlow!list.action?flowTypeId=1&plateType=" + plateType + "'>大车反压</a>");else out.print("<a href='WorkFlow!list.action?flowTypeId=1&plateType=" + plateType + "'>摩托车反压</a>"); %></div>
			...
			<%
			 ...
			 %>
				<tr>
					...
					<td>
						<a href="NumberPlate!findBySmallnessBatchId.action?smallnessBatchNumberId=<%=w.getSmallnessBatchNumber().getId() %>&isRemakes=<%=w.getSmallnessBatchNumber().getIsRemakes() %>&flowTypeId=<%=request.getAttribute("flowTypeId") %>&isDistribute=<%=w.getSmallnessBatchNumber().getIsDistribute() %>&plateType=<%=plateType %>" target="mainFrame">
