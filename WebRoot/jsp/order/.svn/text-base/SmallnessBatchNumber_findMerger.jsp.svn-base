...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<div style="font-size: 14px;">当前位置&nbsp;>&nbsp;生产单管理&nbsp;>&nbsp;<a href="SmallnessBatchNumber!findMerger.action">合并大批号</a></div>
	<%
			Page p = (Page)request.getAttribute("page");
		 %>
	<hr />
	<form action="SmallnessBatchNumber!merger.action" method="post" id="myform">
		<s:token></s:token>
		<div>
			<table  cellpadding="1" cellspacing="1" >
				<tr class="th">
					<td>
						<input style="width: 25px;height:25px;" type="checkbox" onclick="setChkboxStatus('mergers')"/>
					</td>
					<td>
						大批号
					</td>
				...
				%>
					<tr>
						<td>
							<input style="width: 25px;height:25px;" type="checkbox" name="mergers" value="<%=s.getId() %>" />
						</td>
						<td>
		...
		<form action="SmallnessBatchNumber!findMerger.action" method="post" id="fenye">
			...
			<tr>
				<td style="text-align: left;">
					<%
						if(smalls != null && smalls.size() != 0) {
					%>
						<input type="button" onclick="checkWorks('mergers')" class="button" value="合并任务" />
					<%	
						}
					 %>
				</td>
				<td style="text-align: right;">
					...
					%>
							<a href="SmallnessBatchNumber!findMerger.action?page.currentPage=1&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">第一页</a>
					<%
						} else {
					%>
					...
					%>
							<a href="SmallnessBatchNumber!findMerger.action?page.currentPage=<%=p.getCurrentPage()-1 %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">上一页</a>
					<%
						}
					 %>
					...
					%>
							<a href="SmallnessBatchNumber!findMerger.action?page.currentPage=<%=p.getCurrentPage()+1 %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">下一页</a>
					<%						
						}					 
					  %>
					...
					%>
						<a href="SmallnessBatchNumber!findMerger.action?page.currentPage=<%=p.getLastPage() %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">最后一页</a>
					<%
						}
					 %>
