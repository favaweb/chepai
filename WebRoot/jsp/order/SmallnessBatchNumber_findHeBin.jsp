...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<%
		 ...
		 %>
		<div style="font-size: 14px;">当前位置&nbsp;>&nbsp;生产单管理&nbsp;>&nbsp;<a href="SmallnessBatchNumber!findHeBin.action">下发合并</a></div>
		<hr />
		...
		<div>
			<table  cellpadding="1" cellspacing="1"   cellpadding="1" cellspacing="1"  cellpadding="1" cellspacing="1">
				<tr class="th">
					<td>
						<input style="width: 25px;height:25px;" type="checkbox" onclick="setChkboxStatus('goFlows')"/>
					</td>
					<td>
						大批号
					</td>
				...
				%>
					<tr>
						<td>
							<input style="width: 25px;height:25px;" type="checkbox" name="goFlows" value="<%=s.getId() %>" />
						</td>
						<td>
							<%
	...
	<form action="SmallnessBatchNumber!findByMakeStatus.action?makeStatus=0" method="post" id="fenye">
		<table cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="font-size: 12px;">
			...
			<tr>
				...
				<td style="text-align: right;">
					...
					%>
							<a href="SmallnessBatchNumber!findByMakeStatus.action?makeStatus=0&page.currentPage=1&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">第一页</a>
					<%
						} else {
					%>
					...
					%>
							<a href="SmallnessBatchNumber!findByMakeStatus.action?makeStatus=0&page.currentPage=<%=p.getCurrentPage()-1 %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">上一页</a>
					<%
						}
					 %>
					...
					%>
							<a href="SmallnessBatchNumber!findByMakeStatus.action?makeStatus=0&page.currentPage=<%=p.getCurrentPage()+1 %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">下一页</a>
					<%						
						}					 
					  %>
					...
					%>
						<a href="SmallnessBatchNumber!findByMakeStatus.action?makeStatus=0&page.currentPage=<%=p.getLastPage() %>&page.lastPage=<%=p.getLastPage() * p.getPageSize() %>">最后一页</a>
					<%
						}
					 %>
